# 🚀 Обновление Transaction Process через Sharetribe CLI

## 📋 Что будем делать:

Загрузим локальный файл `ext/transaction-processes/assignment-flow-v3/process.edn` в Sharetribe Console через командную строку.

---

## Шаг 1: Установка Sharetribe CLI

Откройте терминал и выполните:

```bash
npm install -g @sharetribe/flex-cli
```

Или через yarn:

```bash
yarn global add @sharetribe/flex-cli
```

**Проверка установки:**

```bash
flex-cli --version
```

Должна появиться версия CLI (например, `6.0.0`).

---

## Шаг 2: Авторизация в Sharetribe

Выполните команду для входа:

```bash
flex-cli login
```

**Что произойдет:**

1. CLI откроет браузер с формой авторизации
2. Войдите в ваш Sharetribe аккаунт (тот же логин/пароль, что и для Console)
3. После успешной авторизации вернитесь в терминал
4. Вы увидите сообщение: `✓ Logged in as your-email@example.com`

---

## Шаг 3: Загрузка процесса в Console

### 3.1. Перейдите в директорию проекта:

```bash
cd /Users/admin/web-template
```

### 3.2. Загрузите процесс:

```bash
flex-cli process push --process assignment-flow-v3 --path ext/transaction-processes/assignment-flow-v3 --marketplace youdoae-dev
```

**⚠️ ВАЖНО:** Используйте путь к **директории**, а не к файлу `process.edn`!

**Пояснение параметров:**

- `--process assignment-flow-v3` - имя процесса в Console
- `--path ext/transaction-processes/assignment-flow-v3` - путь к **директории** с процессом
- `--marketplace youdoae-dev` - ID вашего marketplace

### 3.3. Подтвердите загрузку:

CLI спросит подтверждение:

```
? This will update process 'assignment-flow-v3' in marketplace 'youdoae-dev'. Continue? (Y/n)
```

Введите **`Y`** и нажмите Enter.

---

## ✅ Ожидаемый результат:

```
Version 4 successfully saved for process assignment-flow-v3
```

Или похожее сообщение с номером версии (может быть выше).

**Что изменилось:**

1. ✅ `transition/complete` теперь имеет `:actor :actor.role/provider`
2. ✅ `transition/complete` теперь имеет `:privileged? true`
3. ✅ Provider (Заказчик) может закрывать задания

---

## Шаг 4: Проверка в Console

1. Откройте https://flex-console.sharetribe.com/
2. Перейдите **Build** → **Transaction processes**
3. Найдите **`assignment-flow-v3`**
4. Проверьте, что версия теперь **Version 4** (или выше)

---

## Шаг 5: Тестирование

### 5.1. Перезапустите локальный сервер:

В терминале, где запущен `yarn dev`, нажмите **Ctrl+C** для остановки, затем:

```bash
yarn dev
```

### 5.2. Обновите браузер:

Нажмите **Cmd+Shift+R** (или Ctrl+Shift+R) для полной перезагрузки страницы.

### 5.3. Попробуйте закрыть задание:

1. Авторизуйтесь как **Provider (Заказчик)**
2. Откройте чат с принятым Customer (Исполнителем)
3. Нажмите кнопку **"Закрыть задание"**
4. Подтвердите действие

### ✅ Ожидаемый результат:

**В консоли браузера:**
```
🔄 onComplete: starting transition/complete for transaction ...
✅ onComplete: transition successful
```

**В консоли backend (терминал):**
```
🔍 getTrustedSdk: token exchange successful
🔍 transition-privileged: calling SDK with body: ...
✅ transition-privileged: success, status = 200
```

**В интерфейсе:**
- ✅ Задание успешно закрывается
- ✅ Страница перезагружается
- ✅ Появляется форма для оставления отзыва

---

## 🆘 Решение проблем

### Проблема 1: "Command not found: flex-cli"

**Решение:**

Если установили глобально через npm, но команда не найдена, попробуйте:

```bash
npx @sharetribe/flex-cli --version
```

Тогда используйте `npx @sharetribe/flex-cli` вместо `flex-cli` во всех командах.

### Проблема 2: "Marketplace 'youdoae-dev' not found"

**Решение:**

Проверьте список доступных marketplace:

```bash
flex-cli marketplaces list
```

Используйте правильный ID из списка.

### Проблема 3: "Permission denied"

**Решение:**

Убедитесь, что вы авторизованы:

```bash
flex-cli whoami
```

Если не авторизованы, выполните:

```bash
flex-cli login
```

### Проблема 4: Все еще 403 Forbidden

**Решение:**

1. Проверьте, что версия процесса увеличилась в Console
2. Перезапустите `yarn dev` (Ctrl+C → `yarn dev`)
3. Полностью обновите браузер (Cmd+Shift+R)
4. Очистите кэш браузера (DevTools → Network → Disable cache)

---

## 📚 Полезные команды CLI

```bash
# Посмотреть список процессов
flex-cli process list --marketplace youdoae-dev

# Посмотреть текущую версию процесса
flex-cli process show --process assignment-flow-v3 --marketplace youdoae-dev

# Выйти из аккаунта
flex-cli logout

# Справка по командам
flex-cli help
```

---

## 🔄 Что делать при следующих изменениях процесса

В будущем, когда нужно обновить процесс:

1. Отредактируйте `ext/transaction-processes/assignment-flow-v3/process.edn`
2. Выполните:
   ```bash
   flex-cli process push --process assignment-flow-v3 --path ext/transaction-processes/assignment-flow-v3 --marketplace youdoae-dev
   ```
3. Подтвердите `Y`
4. Перезапустите сервер (`Ctrl+C` → `yarn dev`)

---

**После выполнения всех шагов сообщите результат!** 🚀

