...
export const AsideContent = props => {
  ...
  return (
    <div className={css.asideContent}>
      ...
      {/* Verification status under avatar */}
      <div className={css.verificationStatus}>
        {isVerified ? (
          ...
          <div className={css.notVerifiedBadge}>
            <span className={css.notVerifiedText}>
              <FormattedMessage id="ProfilePage.notVerified" />
            </span>
          </div>
        )}
      </div>

      {/* Instagram under avatar */}
      {instagram && (
        <a
          href={`https://www.instagram.com/${instagram.replace(/^@/, '').replace(/^https?:\/\/(www\.)?instagram\.com\//i, '')}`}
          target="_blank"
          rel="noopener noreferrer"
          className={css.instagramLink}
        >
          <svg className={css.instagramIcon} viewBox="0 0 24 24" fill="currentColor">
          ...
          @{instagram.replace(/^@/, '').replace(/^https?:\/\/(www\.)?instagram\.com\//i, '')}
        </a>
      )}
